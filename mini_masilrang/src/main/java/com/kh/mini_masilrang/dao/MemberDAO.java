...
public class MemberDAO {
    ...
    // 프로필 사진 업데이트
    public boolean profileUpdate(String user_profile, String user_no) {
        int result = 0;
        String sql = "UPDATE MEMBER_INFO SET USER_PROFILE = ? WHERE USER_NO = ?";
        try {
            conn = Common.getConnection();
            pStmt = conn.prepareStatement(sql);
            pStmt.setString(1, user_profile);
            pStmt.setString(2, user_no);
            result = pStmt.executeUpdate();
            System.out.println("profile 업데이트 결과: " + user_no + " - " + user_profile);
